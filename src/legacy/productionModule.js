...
var productionTeardownFns = [];
  ...
  function initCarousel() {
    var currentSlide = 1;
    var root = document.querySelector('.supply-module-main');
    var slides = document.querySelectorAll('.production-carousel .carousel-slide');
    var indicatorsRoot = root ? root.querySelector('.carousel-indicators') : document.querySelector('.carousel-indicators');
    var totalSlides = slides.length || 1;

    function showSlide(slideNum) {
      slides.forEach(function (s) { s.classList.remove('active'); });
      if (indicatorsRoot) {
        indicatorsRoot.querySelectorAll('.indicator').forEach(function (i) { i.classList.remove('active'); });
      }
      var activeSlide = document.querySelector('.production-carousel [data-slide="' + slideNum + '"]');
      if (activeSlide) activeSlide.classList.add('active');
      var activeInd = indicatorsRoot && indicatorsRoot.querySelector('.indicator[data-slide="' + slideNum + '"]');
    ...
    if (indicatorsRoot) {
      indicatorsRoot.addEventListener('click', onIndClick);
      productionTeardownFns.push(function () {
        indicatorsRoot.removeEventListener('click', onIndClick);
      });
    }
    ...
    productionTeardownFns.push(function () { document.removeEventListener('keydown', onKey); });

    var carousel = document.querySelector('.production-carousel');
    var onCarClick = function (e) {
    ...
    if (carousel) {
      carousel.addEventListener('click', onCarClick);
      productionTeardownFns.push(function () { carousel.removeEventListener('click', onCarClick); });
    }
    showSlide(1);
  }
  ...
  function initBackButton() {
    ...
    productionTeardownFns.push(function () { btn.removeEventListener('click', onBack); });
  }
...
export function teardownProductionModule() {
  productionTeardownFns.forEach(function (fn) {
    try { fn(); } catch (e) { /* ignore */ }
  });
  productionTeardownFns = [];
  ...
  delete window.updateProductionModule;
}

export function initProductionModule() {
  teardownProductionModule();
  initCarousel();
  initBackButton();
  initCharts();
